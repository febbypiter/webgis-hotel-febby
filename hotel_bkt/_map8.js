...
$('#tombol').append('<a type="button" id="showlegenda" onclick="legenda()" class="btn btn-success" data-toggle="tooltip" title="Legenda" style="margin-right: 7px;color:black;"><i class="fa fa-eye" style="color:white;"> </i></a>');
}
...
$.ajax({url: server+'_data_hotel_1.php?cari='+ids, data: "", dataType: 'json', success: function(rows){ 
  ...
  // Tombol Angkot
  $('#label_angkot').empty();
  $('#label_angkot').append("<a class='btn btn-success' role=button' data-toggle='collapse'  onclick='angkot_sekitar(\""+ids+"\")' title='Nearby' aria-controls='Nearby' id='btn_angkot'><i class='fa fa-compass' style='color:black;''></i><label>&nbsp Angkot</label></a>")

  // Tombol Gallery
  $('#label_gallery').empty();
  $('#label_gallery').append("<a class='btn btn-success' role=button' data-toggle='collapse'  onclick='galeri(\""+ids+"\")' title='Nearby' aria-controls='Nearby' id='btn_gallery'><i class='fa fa-compass' style='color:black;''></i><label>&nbsp Gallery</label></a>")
...
function cekNearBy(){
  // var kt = document.getElementById('view_kanan_table');
    document.getElementById('judul_table').innerHTML="Nearby";
  ...
  document.getElementById('kanan_table').innerHTML = stringHTML;
  //$('#kanan_table').empty();   
...
function listHotel(){ // Menu Hotel
  ...
  { 
      for (var i in rows){ 
        ...
        $('#kanan_table').append("<tr><td>"+name+"</td><td><a role='button' class='btn btn-success fa fa-info' title='info'  onclick='data_hotel_1_info(\""+id+"\")'></a>&nbsp<a role='button' class='btn btn-success fa fa-taxi' title='Local Transportation' onclick='angkot_sekitar(\""+id+"\")'></a></td></tr>");  
...
function ht_sekitar_user(latitude,longitude,rad){ // TEMPAT WISATA SEKITAR USER
  ...
  $.ajax({url: server+'_sekitar_hotel.php?lat='+latitude+'&lng='+longitude+'&rad='+rad, data: "", dataType: 'json', success: function(rows){ 
    ...
    for (var i in rows){ 
      ...
      $('#kanan_table').append("<tr><td>"+name+"</td><td><a role='button' class='btn btn-success fa fa-info' title='Info' onclick='data_hotel_1_info(\""+id+"\")'></a></td><td><a role='button' style='margin:5px' class='btn btn-success fa fa-taxi' title='Local Transportation' onclick='angkot_sekitar(\""+id+"\")'></a></td></tr>"); 
    }//end for
  }});//end ajax  
...
$.ajax({url: server+'_data_hotel_cari1.php?min='+min+'&max='+max, data: "", dataType: 'json', success: function(rows){ 
  if(rows == null)
  {
    ...
    for (var i in rows){ 
      ...
      var lng = row.lng;
      var lat = row.lat;
      $('#kanan_table').append("<tr><td>"+name+"</td><td><a role='button' class='btn btn-success fa fa-info' title='Info' onclick='data_hotel_1_info(\""+id+"\")'></a></td><td><a role='button' class='btn btn-success fa fa-taxi' title='Local Transportation' onclick='angkot_sekitar(\""+id+"\")'></a></td></tr>");  
...
function kuliner_sekitar(latitude,longitude,rad){ //KULINER SEKITAR ANGKOT
  ...
  $('#table_kanan_culinary').empty();
  $('#table_kanan_culinary').append("<tr><th class='centered'>Culinary Name</th><th class='centered'>Action</th></tr>");
  console.log(server+'_sekitar_culinary_place.php?lat='+latitude+'&lng='+longitude+'&rad='+rad);
...
{ 
    for (var i in rows){ 
      ...
      var lng2 = row.lng2;
      console.log(id);
      route_angkot_1(ids,color);
      $('#table_kanan_angkot').append("<tr><td>"+destination+"</td><td><a role='button' class='btn btn-success fa fa-info' onclick='modal_angkot(\""+ids+"\")'></a><a style='margin-left:10px' role='button' class='btn btn-success fa fa-taxi' onclick='clearangkot();route_angkot(\""+lat2+"\",\""+lng2+"\",\""+lat+"\",\""+lng+"\",\""+ids+"\",\""+id+"\")'></a></td></tr>");         
    }               
